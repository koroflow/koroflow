...
<Tabs groupId="manual-install" items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">```bash npm install @koroflow/elements ```</Tab>
  <Tab value="pnpm">```bash pnpm install @koroflow/elements ```</Tab>
  <Tab value="yarn">```bash yarn add @koroflow/elements ```</Tab>
  <Tab value="bun">```bash bun add @koroflow/elements ```</Tab>
</Tabs>
